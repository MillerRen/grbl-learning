...
当串口接收不为空（有数据）的时候，就从串口获取数据存到串口接收缓冲区，并提供接口`serial_read()`供其他程序从接收缓冲区读取数据； 当有程序通过`serial_write()`把数据写入串口发送缓冲区时，就开启串口发送数据为空的中断，这样当串口发送空闲时就会从串口发送缓冲区获取数据并把它发送给上位机。
