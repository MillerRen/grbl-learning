...
主循环函数`protocol_main_loop()`主要的功能就是从 **GRBL串口RX缓冲区** （不是芯片缓冲区）读取字符串，经过处理后放入 **G代码行缓冲区**，然后交给**G代码解析器**，并且根据各个子模块状态机做出响应。
