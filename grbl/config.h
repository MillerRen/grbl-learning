...
#define CPU_MAP_ATMEGA328P// Arduino Uno（阿杜伊诺）CPU
...
//如果将正常的实时命令集（如状态报告、进给保持、复位和循环启动）移动到扩展设置空间，则需要修改serial.c的RX ISR以适应更改。
...
//取消对该定义的注释，以强制Grbl始终将机器原点设置在归位后的位置，而不管开关方向如何。
...
//通过使其处于禁用状态，您可以在限位开关上执行回零循环，而不必将机器移出限位开关。
...
//Grbl不知道它的位置，并强制用户在继续操作之前执行归位。这一选择迫使无论是否归位，Grbl始终初始化为报警状态。
...
//注意：对于Arduino Unos来说很重要！启用时，Z限制引脚D11和主轴启用引脚D12会互换！
...
//注意：如果禁用，并且为了确保Grbl永远不会更改构建信息行，您还需要启用上面的SETTINGS_RESTORE_ALL宏并从掩码中删除SETTINGS_RESTORE_BUILD_INFO。
